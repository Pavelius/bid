...
void* array::findv(const char* value, unsigned offset, unsigned string_size) const {
	if(!data)
		return 0;
	auto pe = end();
...
int	array::indexof(const void* element) const {
	if(element >= data && element < ((char*)data + element_size * count))
		return (int)(((char*)element - (char*)data) / element_size);
	return -1;
}
...
const void* array::findu(const void* value, unsigned size) const {
	...
	auto s = *((char*)value);
	while(p < pe) {
		p = (char*)memchr(p, s, (unsigned)(pe - p));
