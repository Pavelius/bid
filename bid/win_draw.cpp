...
static LRESULT CALLBACK WndProc(HWND hwnd, unsigned uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_ERASEBKGND:
		if(canvas) {
			...
			video_descriptor.bmp.bmiHeader.biWidth = canvas->width;
			video_descriptor.bmp.bmiHeader.biHeight = -canvas->height;
			video_descriptor.bmp.bmiHeader.biBitCount = canvas->bpp;
			video_descriptor.bmp.bmiHeader.biPlanes = 1;
			if(rc.right != canvas->width || rc.bottom != canvas->height)
				StretchDIBits((void*)wParam,
					0, 0, rc.right, rc.bottom,
					0, 0, canvas->width, canvas->height,
					canvas->bits, &video_descriptor.bmp, DIB_RGB_COLORS, SRCCOPY);
			else
				SetDIBitsToDevice((void*)wParam,
					0, 0, rc.right, rc.bottom,
					0, 0, 0, canvas->height,
					canvas->bits, &video_descriptor.bmp, DIB_RGB_COLORS);
		}
		return 1;
	case WM_CLOSE:
		PostQuitMessage(-1);
