...
void textf(const char* p) {
	...
	while(p[0]) {
		...
		if(match(&p, "#--")) // Header small
			p = textfln(skipsp(p), caret.x, x2, colors::header, metrics::small);
		else if(match(&p, "###")) // Header 3
			p = textfln(skipsp(p), caret.x, x2, colors::header, metrics::h3);
		else if(match(&p, "##")) // Header 2
			p = textfln(skipsp(p), caret.x, x2, colors::header, metrics::h2);
		else if(match(&p, "#")) // Header 1
			p = textfln(skipsp(p), caret.x, x2, colors::header, metrics::h1);
