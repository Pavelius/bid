...
static void ally_help(actionn action, messagen command) {
	for(auto& e : players) {
		...
		if(!e.apply(action, command, false))
			continue;
		an.add(command * 4 + (&e - players), getname(command), e.name());
	}
}
...
void action_roll(actionn type) {
	...
	while(running_scene()) {
		...
		int result = choose_answers(str(getname(ActionRollHeader), player->name(), getname(type), roll_dices), getname(MakeRoll));
